...
Ammo().then((Ammo) => {
  ...
  function loadRyanText() {
    ...
    text_loader.load('./src/jsm/Poppins_Regular.json', function (font) {
      ...
      var textMaterials = [
        new THREE.MeshBasicMaterial({ color: color }), // front基础材质，可以用它赋予几何体一种简单的颜色，或者显示几何体的线框
        new THREE.MeshPhongMaterial({ color: color }), // side这种材质也会考虑光照的影响，并且可以用来创建光亮的物体
      ];

      var geometry = new THREE.TextGeometry("YZY's Word", {
        font: font,//新版本的字体是以json格式保存，我们需要通过异步加载进入页面，所以，必需在加载完字体以后才能够实例化。
        size: 3,//该属性指定文本的大小。默认值为100
        height: 0.5,//该属性指定拉伸的长度（深度）。默认值为50
        curveSegments: 12,//该属性指定拉伸图形时曲线分成多少段。分段数越多，曲线越平滑。默认值为4
        bevelEnabled: true,//如果这个属性设置为true，就会有斜角。默认值为false
        bevelThickness: 0.1,//该属性指定斜角的深度。斜角是前后面和拉伸体之间的倒角。该值定义斜角进入图形的深度。默认值为10。
        bevelSize: 0.11,//该属性指定斜角的高度。默认值为8
        bevelOffset: 0,//表示斜角与原始形状轮廓之间的延伸距离,默认值为0
        bevelSegments: 1,//表示斜角的分段层数,默认值为3
      });

      geometry.computeBoundingBox();//计算图形包围盒尺寸
      geometry.computeVertexNormals();//这是three.js自带的一个计算面法线的方法

      xMid = -0.5 * (geometry.boundingBox.max.x - geometry.boundingBox.min.x);

      geometry.translate(xMid, 0, 0);//修改模型的中心点
      //textGeo为geometry转换成的BufferGeometry
      var textGeo = new THREE.BufferGeometry().fromGeometry(geometry);

      text = new THREE.Mesh(geometry, textMaterials);//创建拍摄对象
      text.position.z = -20;//z轴位置
      text.position.y = 0.1;//y轴位置
  ...
  function loadEngineerText() {
    ...
    text_loader.load('./src/jsm/Poppins_Regular.json', function (font) {
      ...
      text.position.z = -20;
      text.position.y = 1;//0.1
      text.position.x = 20;//14
      text.receiveShadow = true;
      text.castShadow = true;
      scene.add(text);
    });
